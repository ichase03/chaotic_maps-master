...
def cobweb_plot(dynamic, init, iterations, domain=np.linspace(0, 1), **kwargs):
    ...
    ax.plot(domain, func, "b", linewidth=2)
    ...
    for i in range(iterations):
        ...
        ax.plot([x, x], [x, xPrime],"r",linewidth=kwargs["alpha"])

        # from curve to reflection line
        ax.plot([x, xPrime], [xPrime, xPrime], "r", linewidth=1)
...
def cobweb_plot_nD(dynamic, init, var_x_num, iterations, domain=np.linspace(0, 1), **kwargs):
    ...
    if not "var_x_num" in kwargs:
        kwargs["var_x_num"] = 1

    # Plot the map
    # nD_domain = np.vstack((domain,domain) )
    # nD_domain = np.transpose(nD_domain)
    # func = list ( map( dynamic , nD_domain ) )
    # func = np.transpose( func )
    # # ax.plot(domain, dynamic(domain), 'k', linewidth=2) # fails on maps that operate on vectors
    # # func = np.reshape(func, (2, 50))
    # ax.plot(domain, func[0], "k", linewidth=2)
